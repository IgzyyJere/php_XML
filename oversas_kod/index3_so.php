<html>
	<head>
        ...
        <form action="" method="post">
			...
			<br>
            <br>
  <body onload="initialize_map(); add_map_point(-33.8688, 151.2093);" style="width: 90%; margin: 0 auto;">
...
if (empty($rezultat ["data"]["Attachments"][0]["Data"])) {exit('Pošiljka nema fotografiju!');} else {
	...
	if($vrijeme !== null){
		echo 'Datum i vrijeme statusa: '.date("d.m.Y / h:m",strtotime($vrijeme));
	} else{ echo 'Datum i vrijeme statusa: Nema podatka'; }
...
$key_Lat = array_column($new, 'Location');

//echo "<br/> Location : ";
//print_r($key_Lat[0]['Lat']);
//print_r($key_Lat[0]['Long']);

$key1 =  $key_Lat[0]['Lat'];
$key2 = $key_Lat[0]['Long'];

//echo " array search".array_search("Lat",$key_Lat);



//echo "<br/> Location in string : ";
//$ll = json_encode($key_Lat);
//echo $ll;
...
$finalFilter = array_implode($ll);

//echo $finalFilter;
$filter_Lat = 0;
$filter_Lonf = 0;
$filter_index = 0;
$strposition_index = strpos($finalFilter, "Lat");
		...
		if($strposition_index === false){
			...
			//echo $filter_index;
				if($filter_index > 0){
					$filter_index = $filter_index + 5;
					$filter_Lat = substr($finalFilter, $filter_index, 9);
					$filter_index = $filter_index + 17; 
					$filter_Lonf = substr($finalFilter, $filter_index, 8);
				}


				
		echo "<br/>";
		//echo "Reza index (od ...): " .$filter_index;
		echo "<br/>";
		...
		echo "<br/>";
		echo "<br/>";
		//echo "<a href='https://maps.google.com/?q=".$filter_Lat.",".$filter_Lonf."' target='new_tab' class='button2'/>click to google map</a>";
		echo "<br/>";
		echo "<br/>";
...
echo "<br/>";
echo '<img alt="Embedded Image" src="data:image/png;base64,' . $rez1 .  '" style="width:400px" height:300px;/>';
