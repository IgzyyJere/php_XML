...
$upit = "SELECT * FROM novoprojekti WHERE id='".$id."'";
...
selectUpdate ( "Oprema stanova", "opremaStanova", $podaci['opremaStanova'] );
selectUpdate ( "Oprema poslovnih prostora", "opremaPoslovnih", $podaci['opremaPoslovnih'] );
selectUpdate ( "Keramika", "keramika", $podaci['keramika'] );
selectUpdate ( "Sanitarna oprema", "sanitarnaOprema", $podaci['sanitarnaOprema'] );
selectUpdate ( "Sanitarna armatura", "sanitarnaArmatura", $podaci['sanitarnaArmatura'] );
selectUpdate ( "Parket", "parket", $podaci['parket'] );

radioUpdate ( "Noćni čuvar", "nocniCuvar", $podaci['nocniCuvar'] );
radioUpdate ( "Video nadzor", "videoNadzor", $podaci['videoNadzor'] );

selectUpdate ( "Grijanje", "grijanje", $podaci['grijanje'] );
radioUpdate ( "Protupr. vrata", "protuprovala", $podaci['protuprovala'] );

multiRadioUpdate ( "Telefon", "telefon", $podaci['telefon'] );
...
?>
<input type="hidden" name="izmjenaProjekta" value="1">
