...
if ( $_POST['napravi'] == 'unos' ) {
    ...
    if ( $_POST['prima'] ) {
        ...
        $upit = "INSERT INTO `".$tabela."`
                ( `id`, `naslov`, `tekst`, `prima`, `poslao`, `datum`, `spojiTabela`, `spojiId`, `datumUnosa` )
                VALUES
                ( '', '".$_POST['naslov']."', '".$_POST['tekst']."', '".$_POST['prima']."',
                '".$korisnik['id']."', '".$_POST['godina']."-".$_POST['mjesec']."-".$_POST['dan']."',
                '".$_POST['spojiTabela']."','".$_POST['spojiId']."', CURDATE()   )";
        mysql_query ( $upit );
...
while ( $podi = mysql_fetch_assoc ( $od ) ) {
    ...
    echo '<div class="',$back,' prikazFormLine">
        ...
        ',$dan,'.',$mjesec,'.',$godina,' - ',$podi['naslov'],'</div><div class="pomocniNav"><a href="" title="prikaži podsjetnike" class="prikaziPodsjetnik" ref="',$podi['id'],'""><img src="/vivo2/ikone/bell.png"></a></div>
    </div>';
